# Modern Login Components

This directory contains modern, premium-styled login components for the application.

## Components

### PhoneLoginModern.jsx

A sleek phone number login form with:

- Phone number input with formatting
- SMS verification code flow
- Code display for testing
- Back navigation
- Alternative login options

## Design Principles

- Premium, minimal aesthetic
- Centered layout with subtle gradients
- Consistent color scheme using brand colors (#8BC34A and #FFC107)
- Smooth animations and transitions
- Fully responsive for all device sizes
- Accessible form elements with proper labeling

## Usage

Import the components directly:

```jsx
import { PhoneLoginModern } from "./components/Login";
```

Or import individually:

```jsx
import PhoneLoginModern from "./components/Login/PhoneLoginModern";
```
