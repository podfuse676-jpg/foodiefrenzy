# Start Foodie Frenzy Frontend and Admin Panel
...
Set-Location -Path "$currentDir\FOODIEFRENZY\frontend"
Start-Process -FilePath "npm" -ArgumentList "run", "dev" -WindowStyle Normal

# Start admin service
Write-Host "Starting admin service on port 5174..." -ForegroundColor Yellow
Set-Location -Path "$currentDir\FOODIEFRENZY\admin"
Start-Process -FilePath "npm" -ArgumentList "run", "dev" -WindowStyle Normal
...
Write-Host ""
Write-Host "To stop these services, close the command prompt windows that opened," -ForegroundColor Yellow
Write-Host "or run stop-all.bat to stop all Node.js processes." -ForegroundColor Yellow