...
import { FiArrowLeft, FiPackage, FiTruck, FiCheckCircle, FiClock, FiMapPin, FiCreditCard, FiDollarSign, FiUser } from 'react-icons/fi';
...
const OrderDetail = () => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center text-green-400 text-xl">
  ...
  return (
    <div className="min-h-screen bg-gray-900 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gray-800 rounded-3xl p-8 shadow-2xl border border-green-500/20">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div className="flex items-center gap-4">
              <div className={`${status.bg} ${status.color} flex items-center gap-2 text-sm px-4 py-2 rounded-lg border border-green-500/30`}>
                {status.icon}
              ...
              <div className="bg-gray-700/50 rounded-2xl p-6">
                ...
                <div className="mt-6 pt-6 border-t border-green-500/20">
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="text-green-100">${typeof order.subtotal === 'number' ? order.subtotal.toFixed(2) : '0.00'} CAD</span>
                  </div>
                  <div className="flex justify-between mb-2">
                    <span className="text-green-100">Tax (5%):</span>
                    <span className="text-green-100">${typeof order.tax === 'number' ? order.tax.toFixed(2) : '0.00'} CAD</span>
                  </div>
                  <div className="flex justify-between mb-2">
                    <span className="text-green-100">Shipping:</span>
                    <span className="text-green-100">${typeof order.shipping === 'number' ? order.shipping.toFixed(2) : '0.00'} CAD</span>
                  </div>
                  {order.codFee > 0 && (
                    <div className="flex justify-between mb-2">
                      <span className="text-green-100">COD Fee:</span>
                      <span className="text-green-100">${typeof order.codFee === 'number' ? order.codFee.toFixed(2) : '0.00'} CAD</span>
                    </div>
                  )}
                  <div className="flex justify-between mt-4 pt-4 border-t border-green-500/30">
                    ...
                    <span className="text-lg font-bold text-green-300">${typeof order.total === 'number' ? order.total.toFixed(2) : '0.00'} CAD</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-gray-700/50 rounded-2xl p-6">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between">
                    <span className="text-green-400/60">Order ID:</span>
                    <span className="font-mono text-green-100">#{order._id?.slice(-8)}</span>
                  </div>
                  
                  <div className="flex justify-between">
                    <span className="text-green-400/60">Order Date:</span>
                  ...
                  <div className="flex justify-between">
                    <span className="text-green-400/60">Items:</span>
                    <span className="text-green-100">{totalItems}</span>
                  </div>
                  
                  <div className="flex justify-between">
                    <span className="text-green-400/60">Order Status:</span>
